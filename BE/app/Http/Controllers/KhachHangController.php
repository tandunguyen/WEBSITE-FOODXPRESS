...
class KhachHangController extends Controller
{
    ...
    public function Register(Request $request)
    {
        ...
        return response()->json([
            'status'  => 1,
            'message' => 'Đã thêm mới khách hàng  ' . $request->ho_va_ten . ' thành công.'
        ]);
    }
    ...
    //     // Tạo mã 6 số (hoặc dùng Str::random(32) cũng được)
    ...
    //     // TODO: Gửi email thực tế ở đây (Mail::to($kh->email)->send(...))
    //     // Tạm thời trả về code để test (sau này bỏ đi)
    //     return response()->json([
    //         'status'  => 1,
    //         'message' => 'Đã gửi mã xác nhận quên mật khẩu!',
    //         'code_test' => $code, // chỉ dùng để test
    //     ]);
    // }
    ...
    public function guiMaQuenMatKhau(Request $request)
    {
        ...
        return response()->json([
            ...
            // 'code_test' => $code, // nếu muốn debug vẫn để
        ]);
    }
    // 2) Xác thực mã
    ...
    public function datLaiMatKhau(Request $request)
    {
        ...
        // Nếu bạn đang lưu plain password như hàm doiMatKhau:
        $kh->update([
            'password'   => $request->password,
            'hash_reset' => null, // reset lại để mã cũ không dùng nữa
        ]);

        // Nếu sau này dùng hash:
        // $kh->update([
